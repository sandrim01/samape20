// API Wrapper para versão web (usa fetch em vez de Electron IPC)
window.api = {
  login: (dados) => fetch('/api/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  criarUsuario: (dados) => fetch('/api/criar-usuario', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarUsuarios: () => fetch('/api/listar-usuarios', { method: 'POST' }).then(r => r.json()),

  criarCliente: (dados) => fetch('/api/criar-cliente', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarClientes: () => fetch('/api/listar-clientes', { method: 'POST' }).then(r => r.json()),

  criarMaquina: (dados) => fetch('/api/criar-maquina', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarMaquinas: (cliente_id) => fetch('/api/listar-maquinas', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ cliente_id })
  }).then(r => r.json()),

  criarOS: (dados) => fetch('/api/criar-os', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarOS: (filtros) => fetch('/api/listar-os', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(filtros || {})
  }).then(r => r.json()),

  atualizarOS: (id, dados) => fetch('/api/atualizar-os', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, dados })
  }).then(r => r.json()),

  criarPeca: (dados) => fetch('/api/criar-peca', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarPecas: () => fetch('/api/listar-pecas', { method: 'POST' }).then(r => r.json()),

  criarVenda: (dados) => fetch('/api/criar-venda', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarVendas: () => fetch('/api/listar-vendas', { method: 'POST' }).then(r => r.json()),

  criarContaReceber: (dados) => fetch('/api/criar-conta-receber', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarContasReceber: (filtros) => fetch('/api/listar-contas-receber', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(filtros || {})
  }).then(r => r.json()),

  registrarPagamentoReceber: (id, data) => fetch('/api/registrar-pagamento-receber', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, data_pagamento: data })
  }).then(r => r.json()),

  criarContaPagar: (dados) => fetch('/api/criar-conta-pagar', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dados)
  }).then(r => r.json()),

  listarContasPagar: (filtros) => fetch('/api/listar-contas-pagar', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(filtros || {})
  }).then(r => r.json()),

  registrarPagamentoPagar: (id, data) => fetch('/api/registrar-pagamento-pagar', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, data_pagamento: data })
  }).then(r => r.json()),

  obterEstatisticas: () => fetch('/api/obter-estatisticas', { method: 'POST' }).then(r => r.json())
};
    ...
    <div class="login-container">
      <div class="login-box">
        <div class="login-header">
          ...
          <h1 class="login-title">SAMAPEOP</h1>
          <p class="login-subtitle">Sistema de Gerenciamento de Manutenção</p>
        </div>
  ...
  return `
    <div class="sidebar">
      <div class="sidebar-header">
        ...
        <div class="sidebar-title">SAMAPEOP</div>
      </div>
...
function showNovaOSModal() {
  ...
  const mecanicosOptions = AppState.data.usuarios
    .filter(u => u.cargo === 'MECANICO' || u.cargo === 'ADMIN')
    .map(u => `<option value="${u.id}">${u.nome}</option>`)
    .join('');

  showModal('Nova Ordem de Serviço', `
    <div class="form-grid">
      <div class="form-group">
        <label class="form-label">Cliente *</label>
        <select class="form-input" id="modal-os-cliente" required>
          <option value="">Selecione...</option>
          ${clientesOptions}
        </select>
      </div>
      <div class="form-group">
        ...
        <select class="form-input" id="modal-os-maquina" required>
          <option value="">Selecione o cliente primeiro</option>
        </select>
      </div>
      <div class="form-group">
        <label class="form-label">Mecânico *</label>
        <select class="form-input" id="modal-os-mecanico" required>
          <option value="">Selecione...</option>
          ${mecanicosOptions}
        </select>
      </div>
    ...
    <div class="form-group">
      ...
      <textarea class="form-input" id="modal-os-problema" required></textarea>
    </div>
  `, async () => {
    const dados = {
      cliente_id: document.getElementById('modal-os-cliente').value,
      maquina_id: document.getElementById('modal-os-maquina').value,
      mecanico_id: document.getElementById('modal-os-mecanico').value,
      descricao_problema: document.getElementById('modal-os-problema').value
    };

    const result = await window.api.criarOS(dados);
    if (result.success) {
      await loadOrdensServico();
      closeModal();
      render();
    } else {
      alert('Erro ao criar OS: ' + result.message);
    }
  });

  // Carregar máquinas quando cliente for selecionado
  document.getElementById('modal-os-cliente').addEventListener('change', async (e) => {
    const clienteId = e.target.value;
    if (clienteId) {
      const result = await window.api.listarMaquinas(parseInt(clienteId));
      if (result.success) {
        const select = document.getElementById('modal-os-maquina');
        select.innerHTML = '<option value="">Selecione...</option>' +
          result.maquinas.map(m => `<option value="${m.id}">${m.modelo}</option>`).join('');
      }
    }
  });
}
